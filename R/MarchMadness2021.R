...
#Project: Kaggle March Madness Competition 2021
...
save(cv_results, file = str_c(root, "MM2021_cv_results.RData"))
...
save(final_models, file = str_c(root, "MM2021_final_models.RData"))
  ...
  imap(
    function(.models, .tourney) {
      ...
      pluck(test_model_dat, .tourney) %>%
        ...
        imap(
          function(.preds, .framework) {
            ...
            write_csv(
              .preds,
              path = str_c(root, "MarchMadness2021_Predictions_", .tourney, "_", .framework, "Kaggle_", Sys.Date(), ".csv")
            )
      ...
      pluck(test_model_dat, .tourney) %>%
        ...
        #Write to file
        write_csv(
          path = str_c(root, "MarchMadness2021_Predictions_", .tourney, "_Lookup.csv")
        )
